...
CREATE TABLE CUSTOMER_T (
    ...
    CUST_AGE    NUMBER(3)           CHECK(CUST_AGE >= 0 AND CUST_AGE <= 120),
                              --    CHECK(CUST AGE BETWEEN 0 AND 120),
...
/*
    ...
    4. 칼럼 이름 바꾸기    : ALTER TABLE 테이블명 RENAME COLUMN   기존칼럼 TO 신규칼럼명
    5. 테이블 이름 바꾸기  : ALTER TABLE 테이블명 RENAME TO 신규테이블명
*/


-- 1. BANK_T에 연락처(BANK_TEL) 칼럼을 추가하시오.
ALTER TABLE BANK_T ADD BANK_TEL VARCHAR2(20 BYTE);
...
ALTER TABLE CUST_T ADD GRADE VARCHAR2(10 BYTE) CHECK(GRADE = 'VIP' OR GRADE = 'GOLD' OR GRADE = 'SILVER' OR GRADE = 'BRONZE');
                                            -- CHECK(GRADE IN('VIP', 'GOLD', 'SILVER', 'BRONZE'));
-- 테이블 명 바꾼 이유 : 7번에서 테이블 명 바꿔서 다시 함.
...
/*
    ...
    1. PK
        1) 추가 : ALTER TABLE 테이블명 ADD CONTRAINT 제약조건명 PRIMARY KEY(칼럼명)
        2) 삭제  
    ...
    2. FK
        ...
        4) 활성화   : ALTER TABLE 테이블명      ENABLE   CONSTRAINT 제역조건명
*/
