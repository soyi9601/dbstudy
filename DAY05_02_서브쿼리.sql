...
/*
    ...
    2. 서브 쿼리의 실행 결과를 임시 테이블의 형식을 FROM 절에 두고 사용한다.
...
-- 2. 연봉 TOP 10을 조회하시오.
SELECT A.*
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY SALARY DESC) AS RN, EMPLOYEE_ID, SALARY, HIRE_DATE
          FROM EMPLOYEES) A
...
/*
    ...
    3. 형식
        INSERT INTO 테이븖여(칼럼1, 칼럼2, ...) (SELECT 칼럼1, 칼럼2, ...)
*/
